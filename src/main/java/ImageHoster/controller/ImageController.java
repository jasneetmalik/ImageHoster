...
public class ImageController {
    ...
    //CHANGED REQUEST MAPPING TO INCLUDE ID AND RETRIEVING RECORD BY ID INSTEAD
    @RequestMapping("/images/{id}/{title}")
    public String showImage(@PathVariable("id") Integer id, Model model) {
        Image image = imageService.getImage(id);
        model.addAttribute("image", image);
        model.addAttribute("tags", image.getTags());
        List<Comment> comments = commentService.getAllComments(id); // Added all comments in the list so HTML view can render all comments with loop
